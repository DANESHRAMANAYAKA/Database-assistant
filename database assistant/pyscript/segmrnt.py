...
def check_Entity_And_Relatios():
        ...
        with open("D:\\sliit\\4th year\\CDAP\\proj_workspace\\ConsoleApplication2\\dist\\ScriptsegmentOutput.txt","r") as ins:
...
def filter_Entites(token):
    ...
    file = open("D:\\sliit\\4th year\\CDAP\\proj_workspace\\ConsoleApplication2\\dist\\entites.txt", "a+")
...
def filter_attri(token):
    ...
    for sentence in sentences:
        ...
        for word, pos in nltk.pos_tag(nltk.word_tokenize(str(sentence))):
            if (pos == 'NN' or pos == 'NNP' or pos == 'NNS' or pos == 'NNPS'):
                nouns.append(word)
                #filter entity11111111111111111111111111111111111111111111111111111111
                if (count == 0):
                    count = count + 1
    ...
    file = open("D:\\sliit\\4th year\\CDAP\\proj_workspace\\ConsoleApplication2\\dist\\attrybuts.txt", "a+")
    file.write(str(nouns))
    file.write("\n")
...
def analizer_attr_entity():


     attrybuts = []
     entites=[]

     f = open('D:\\sliit\\4th year\\CDAP\\proj_workspace\\ConsoleApplication2\\dist\\entites.txt', 'r')
     entites = f.readlines()
     entites = [s.strip('\n') for s in entites]
     f.close()
     #print(entites)

     f = open('D:\\sliit\\4th year\\CDAP\\proj_workspace\\ConsoleApplication2\\dist\\attrybuts.txt', 'r')
     attrybuts = f.readlines()
     attrybuts = [s.strip('\n') for s in attrybuts]
     f.close()
     #print(attrybuts)
     print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
